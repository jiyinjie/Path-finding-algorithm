...
int nodes = 0; 
 for ( Vertex v : vertices.values() ) {
    v.visited = false;
    v.cost = 999999;
    v.estimate = v.cost + h.fn(v, goal);
    }
  start.cost = 0;
  start.estimate = start.cost + h.fn(start,goal);
  start.parent = null;
  PriorityQueue<Vertex>
      fringe = new PriorityQueue<Vertex>(20,
        new Comparator<Vertex>() {
  ...
  while ( ! fringe.isEmpty() ) {
    Vertex v = fringe.remove();  // lowest-cost
    //System.out.println("removing "+ v.str());
    nodes++;
    if( v == goal) break;
    if ( ! v.visited )
      { v.visited = true;
...
public static final Heuristic randombs = new Heuristic() {
  	public Integer fn(Vertex from, Vertex to){
	    ...
	    return ((Double)(distance*rng.nextDouble())).intValue()
	    ;}
 };
...
public Cons pathto(String city) {
	Vertex target = vertex(city);
	if (target.parent()== null)
		return Cons.list(target.str());
	else return Cons.append(pathto(target.parent().str()),Cons.list(target.str()));
}
